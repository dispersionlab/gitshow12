...
Nois | PINK <--> *FM1*, **LFO-1**  |  /  | Pink noise has more bass, and its intensity decreases by -3 dB/octave.     
Nois |  RED <--> /  |  / | Red noise has even more bass, and its intensity decreases by -6 dB/octave.
Nois |  VIOL <--> /  |  / | Violet noise has even more treble, and its intensity increases by +6 dB/octave.
Nois |  BLUE <--> /  |  / | Blue noise has more treble than white noise, and its intensity increases by +3 dB/octave.
...
VCO-1 | ANLG/DIG <--> **ANLG** |  /  | ANLG position, analog-modeled waveforms are used. In most cases, like subtractive synthesis, analog waveforms are often preferred due to their imperfect and “more musical” harmonics. ![VOL-1_ANLG_01](https://vcvrack.com/images/Fundamental/VCO-sine.png)![VOL-1_ANLG_02](https://vcvrack.com/images/Fundamental/VCO-tri.png)![VOL-1_ANLG_03](https://vcvrack.com/images/Fundamental/VCO-saw.png)![VOL-1_ANLG_04](https://vcvrack.com/images/Fundamental/VCO-square.png)
VCO-1 | **FREQ** knob |  43.257  | The pitch of each oscillator is controlled by the FREQ knob, with a range of 9 octaves. 
VCO-1 | HARD/SOFT <--> **SOFT** |  43.257  | SOFT position, soft-sync is used instead, which reverses the direction of the waveform when the sync signal passes through 0V. This results in a “softer” effect with fewer harmonics than hard-sync.
VCO-1 | **FINE** knob |  -0.123  | FINE knob (only on VCO-1), with an additional range of 2 semitones. 
VCO-1 | **P.WIDTH** knob |  75.725  | P. WIDTH knob adjusts the pulse width (aka duty cycle) of the square waveform
VCO-1 | **FM CV** knob |  63  | PWM input and its PWM CV attenuator knob allow external pulse width control.               
VCO-1 | **PWM.CV** knob |  58.5  | PWM input and its PWM CV attenuator knob allow external pulse width control.
VCO-1 | V/OCT  |  /  | FINE
VCO-1 | FM <-> *WHT*,**NOIS**  |  /  | FM input applies an additional pitch adjustment with an included FM CV attenuator knob. With the FM CV knob hard-right, the FM input functions similarly to the V/OCT input.
VCO-1 | SYNC  |  /  | SYNC input applies hard-syncing to the waveforms by resetting the phase of the waveform when the sync signal passes upward through 0V. If an audio-rate signal is patched into the SYNC input, the result is a waveform having characteristics of both the internal and sync frequencies.           
VCO-1 | PWM <-> *SIN*,**VCO-1**  |  /  | PWM input and its PWM CV attenuator knob allow external pulse width control. The following are examples of a digital square wave with 1%, 25%, 50%, and 75% pulse width. ![PWM Pulse Width_01](https://vcvrack.com/images/Fundamental/VCO-01square.png)    ![PWM Pulse Width_02](https://vcvrack.com/images/Fundamental/VCO-25square.png)       ![PWM Pulse Width_03](https://vcvrack.com/images/Fundamental/VCO-50square.png)![PWM Pulse Width_04](https://vcvrack.com/images/Fundamental/VCO-75square.png)
VCO-1 |  SIN <-> PWM + *Attack*,**ADSR**; + *Sustain*,**ADSR**                         |  *Attack*,**ADSR**:51.531; *Sustain*,**ADSR**: 60.35;  | SIN(sine) is one of the four output waveform in VCO-1 (traditional analogy/digital oscillar).
VCO-1 | TRI <-> *CV*,**ADSR**  |  DEC:263.03  | TRI (triangle)is one of the four output waveform in VCO-1 (traditional analogy/digital oscillar).   
VCO-1 | SAW <-> *CV*,**DELAY**  |  FDBK:72.7  | SAW (sawtooth)s one of the four output waveform in VCO-1 (traditional analogy/digital oscillar).                         
VCO-1 | SQR <-> *CV*,**DELAY**  |  TIME:0.26062  | SQR (square)is one of the four output waveform in VCO-1 (traditional analogy/digital oscillar). 
...
LFO-1 | UNI/BI <-> **BI**  |  /  |  The UNI/BI switch toggles between unipolar (0V to 10V) and bipolar (-5V to 5V) output voltages.
LFO-1 | FREQ | 0.014751 |  /
LFO-1 | 0/180 <-> **180** | / | Phrase Range
LFO-1 | FM 1 CV | 68.4 | /
LFO-1 | P.WIDTH | 67.199 | /
LFO-1 | FM 2 CV | 67.199 | /
LFO-1 | PWM CV | 0 |/
LFO-1 | FM1 <-> *SIN*,**VCO-1** | / | /
LFO-1 | FM2 <-> / | / |
LFO-1 | RESET | / | /
LFO-1 | SIN, *CV*,**ADSR** | REL:1958.9 | /
LFO-1 | TRI, *Gate*,**ADSR** | / | /
LFO-1 | SAW, *IN*,**VCF** | / | /
LFO-1 | SQR | / | /
ADSR | / | / | ADSR module manipulates the variables of Attack, Decay, Sustain, and Release easily. 
ADSR | CV,Attack <-> *SIN*,**VCO-1** | 51.531 | Attack in ADSR is the amount of time it takes from when you first press a key until you hear the full volume of a note's sound.
ADSR | CV, Decay <-> *TRI*,**VCO-1** | 263.03 | Decay is the amount of TIME it takes for a sound to go from its maximum volume to its minimum volum while still holding a key down.
ADSR | CV, Sustain <-> *SIN*,**VCO-1** | 60.45 | Sustain is the difference in volume from the beginning of sound to the minimum level of sound while still pressing a key down. It is useful to consider Sustain and Decay at the same time as they are closely connected. 
ADSR | CV, Release <-> *SIN*,**LFO-1** | 1958.9 | Release is the amount of time it takes for the sound to go to silence after a note has been released. The image here has a very long slow Release.
ADSR | Gate <-> *TRI*,**LFO-1** | / | GATE output generates 10V
ADSR | RETRIG <-> / | / | Socket " is the ADSR’s retrigger input, which can be connected, for instance, to the output from an LFO. That means that while the gate is open, the envelope re-triggers every time it senses a pulse from the LFO.
ADSR  | OUT <-> *IN*,**DELAY**; + *IN2*,**SPRING REVERB** | / | /
...
DELAY | CV, Time <-> *SQR*,**VCO-1** | 0.26062 | Time allows us to adjust the time between repeated sounds. Further to the left can take the sounds down to the milliseconds and to the right provides a delay seconds in duration. Rotating the control to the right can create such fast repeated sounds that it can result in some fascinating textures.
DELAY | CV, Feedback <-> *SAW*,**VCO-1** | 72.7 | If Time controls the lenght of time between repeated sounds Feedback controls the time it takes for the repeated sounds to fade away to silence. Turning the control all the way to the left will have the sound fade away instantly and all the way to the right will have the sound repeating on and on with now decrease in volume level
DELAY | CV, Colour <-> / | 70.1 | Colours of noise, each with a different characteristic and frequency response.
DELAY | CV, Mix <-> / | 90.05 | /
